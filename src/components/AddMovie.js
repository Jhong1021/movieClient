...
function addMovie(e) {
		...
		fetch(`https://movieappapi-xi7a.onrender.com/movies/addMovie`, {
			method: "POST",
			headers: { 
		...
		.then(data => {
			...
			Swal.fire({
				...
				Swal.fire({
					title: 'Error in adding the movie',
					icon: 'error'
				})
	...
	return (
	<>
		<Button className="p-3 mb-2" variant="primary" size="sm" onClick={() => openM()}>Add Movie</Button>
		...
		<Form className="p-5" onSubmit={e => addMovie(e)}>
	      ...
	      <Form.Group className="mb-3" controlId="formBasicPassword">
	        ...
	        <Form.Control type="text" placeholder="Enter Year" value={year} onChange={e => {setYear(e.target.value)}} required/>
	      </Form.Group>
